...
def search_path(closest_points,minimum,list_of_fields,iteration):
    ...
    for j in range(iteration):
        ...
        if (x_goal[0] - 2) <= closest_points[0][0] <= (x_goal[0]  + 2) and (y_goal[0]  - 2) <= closest_points[0][1] <= (y_goal[0]  + 2):
...
def center_origin(surf, p):
    return (p[0]  + 80, p[1] + surf.get_height() - 47)
...
q.append((x_goal[0] , y_goal[0] , dist))  # adding initial values to the queue (goal coordinates and 0 as a current distance)
...
start_coordinates1.append((x_start[0], y_start[0]))
closest_points.append((x_start[0], y_start[0]))
minimum = 600

#pygame.draw.rect(surf, (0,255,255), pygame.Rect(80, 444-47, 1, 1), 3)

generated_path = search_path(closest_points, minimum, new_f, 400)
optimal = search_min_distances(generated_path, (x_goal[0], y_goal[0]))
draw_path(start_coordinates1, optimal)

screen.blit(surf, center_origin(surf, (0, 0)))
pygame.display.flip()
