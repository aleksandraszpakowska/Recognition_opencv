...
def path_smoothing(path, iter_number):
    ...
    x0 = x_list_x.copy()
    y0 = y_list_y.copy()
    alpha = .3 # decided how fast we moving away from base position
    #to avoid the straight line we are moving new position into direction of the base position
    beta = .3
    new_path = []
    #iter number - number of smoothing operations
    for i0 in range(0, iter_number):
        for i in range(1, len(path) - 1):
            # part1
            # we are getting the three points
